package com.todoseventos.todos_eventos.enuns;

public class SuccessMessages {

    public static final String DEPOSITO_REALIZADO = "Depósito realizado com sucesso!";
    public static final String CADASTRO_EVENTO = "Cadastro realizado com sucesso. Seu evento já está em divulgação!";
    public static final String EVENTO_ENCERRADO = "Evento encerrado com sucesso. Dentro de alguns minutos, você receberá um email!!";
    public static final String EVENTO_ATUALIZADO = "Evento atualizado com sucesso!";
    public static final String EXCLUIR_EVENTO = "Evento excluído com sucesso";
    public static final String CADASTRO_CLIENTE = "Cliente cadastrado com sucesso!";
    public static final String CLIENTE_ATUALIZADO = "Cliente atualizado com sucesso!";
    public static final String COMPRA_REALIZADA = "Compra realizada com sucesso. Dentro de alguns minutos, você receberá um email!";
    public static final String CADASTRO_ADMINISTRADOR = "Administrador cadastrado com sucesso!";

    private SuccessMessages(){
    }
}
